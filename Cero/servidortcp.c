...
void smtp(int s, char *buf,char *hostname);


//TODO: Hacer log con el numero de puerto efímero de cada cliente
...
char *argv[];
{
    ...
    if (s_TCP == -1) {
		perror(argv[0]);
		fprintf(stderr, "%s: no se ha podido aceptar al socket\n", argv[0]);
		exit(1);
	}
	switch (fork()) {
...
{
	char msg[9] = "Prueba\r";
	if (send(s, msg, TAM_BUFFER, 0) != TAM_BUFFER) errout(hostname);
}
